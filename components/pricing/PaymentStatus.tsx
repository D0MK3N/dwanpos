...
export default function PaymentStatusModal({ orderId, isOpen, onClose }: PaymentStatusModalProps) {
  ...
  const getStatusInfo = () => {
    switch (status) {
      case 'COMPLETED':
        return { color: 'text-green-500', bgColor: 'bg-green-500/10', message: 'Payment Completed Successfully!', icon: '✅' };
      case 'APPROVED':
        return { color: 'text-blue-500', bgColor: 'bg-blue-500/10', message: 'Payment Approved', icon: '⏳' };
      case 'CREATED':
        return { color: 'text-yellow-500', bgColor: 'bg-yellow-500/10', message: 'Payment Created', icon: '🔄' };
      case 'error':
        return { color: 'text-red-500', bgColor: 'bg-red-500/10', message: 'Error Checking Status', icon: '❌' };
      default:
        return { color: 'text-gray-500', bgColor: 'bg-gray-500/10', message: 'Unknown Status', icon: '❓' };
    }
  };
